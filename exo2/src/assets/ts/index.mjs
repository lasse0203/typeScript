"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const contacts = [];
// Compteur de contacts
let contactCount = 0;
// Sélection des éléments du formulaire et de la liste de contacts
const contactForm = document.getElementById("contact-form");
const contactList = document.getElementById("contact-list");
// Écouteur d'événement pour soumettre le formulaire
contactForm.addEventListener("submit", (e) => {
    e.preventDefault();
    const firstName = document.getElementById("first-name").value;
    const lastName = document.getElementById("last-name").value;
    const email = document.getElementById("email").value;
    const phone = document.getElementById("phone").value;
    if (firstName && lastName && email && phone) {
        // Crée un contact unique avec un ID
        const contact = {
            id: contactCount,
            firstName,
            lastName,
            email,
            phone
        };
        // Ajoute le contact au tableau de contacts
        contacts.push(contact);
        // Met à jour le compteur de contacts
        contactCount++;
        // Ajoute le contact à la liste
        displayContact(contact);
        // Réinitialise le formulaire
        contactForm.reset();
    }
});
// Fonction pour afficher un contact dans la liste
function displayContact(contact) {
    const contactItem = document.createElement("div");
    contactItem.classList.add("contact-item");
    contactItem.innerHTML = `
        <span>ID: ${contact.id}</span>
        <span>Prénom: ${contact.firstName}</span>
        <span>Nom: ${contact.lastName}</span>
        <span>Email: ${contact.email}</span>
        <span>Téléphone: ${contact.phone}</span>
        <button class="delete-button" data-id="${contact.id}">Supprimer</button>
    `;
    contactList === null || contactList === void 0 ? void 0 : contactList.appendChild(contactItem);
    // Écouteur d'événement pour le bouton de suppression
    const deleteButton = contactItem.querySelector(".delete-button");
    deleteButton.addEventListener("click", () => {
        // Supprime le contact du tableau
        contacts.splice(contacts.findIndex(c => c.id === contact.id), 1);
        // Supprime l'élément de la liste
        contactList === null || contactList === void 0 ? void 0 : contactList.removeChild(contactItem);
    });
}
// Chargement initial de la liste des contacts
contacts.forEach(displayContact);
